...
单场景命令从 CLI 到结构化日志和最终报告写入的调用链如下：
...
src/client/cli.ts
  buildProgram()
    .command("run").action(...)
      parseProtocol()
      makeOptions()
      createFileRunLogger(reportDir, options)
      runOne(options, { logger })
        ...
        runWithResilience(options, runner, { logger })
          ...
          logger.log(run_finished)
            createFileRunLogger -> buildRunReport() -> writeJsonReport()
      buildRunReport(options, outcome)
      formatHumanReport() | JSON.stringify(report)
```
...
| `src/client/cli.ts` | `makeOptions()` | 把 CLI flags 转为 `RunOptions`，包括 `scenario`、`mode`、超时、fallback、报告目录 |
| `src/client/cli.ts` | `buildProgram().command("run").action(...)` | CLI 外层组装点：创建文件 logger，把 `RunOptions` 和 logger 传给流程方法 |
| `src/client/cli.ts` | `runOne()` | 单次请求流程方法：选择协议 Runner，调用韧性策略；只接收 `RunLogger` 抽象，不创建文件 logger、不直接写报告 |
...
| `src/client/resilience/policy.ts` | `makeOutcome()` | 生成流程结果 `RunOutcome`，只包含 request id、输出、问题、缓解动作、状态和 timing，不包含报告维度 |
| `src/client/resilience/normalizeError.ts` | `normalizeProviderError()` | 合并 `classifyError()` 与 `retry-after` 提取结果 |
| `src/client/reports.ts` | `buildRunReport()` | 把 `RunOptions + RunOutcome` 投影为 `RunReport`，补齐 `protocol`、`mode`、`scenario`、`use_case_id` |
| `src/client/reports.ts` | `createFileRunLogger()` | 返回文件 logger，消费 `run_finished` outcome 事件并写出 `reports/<request_id>.json` |
| `src/client/reports.ts` | `writeJsonReport()` | 文件 logger 内部复用的 JSON 写入函数 |
...
如需机器可读报告，在命令末尾加 `--json`。无论是否加 `--json`，CLI 都会注入文件 logger。策略层发出 `run_finished` 日志事件后，文件 logger 会写出 `reports/<request_id>.json`。请求方法本身不负责文件写入。
...
阅读后续每个“客户端”小节时，可以按下面这条链路定位代码：**CLI 组装 `RunOptions` -> 协议 Runner 调官方 SDK 读取响应 -> Runner 把服务端响应压缩成 `SdkRunResult` 或带 partial state 的错误 -> `runWithResilience()` 分类并决策 -> `reports.ts` 把 outcome 投影成报告**。

| 层级 | 代码位置 | 读什么 |
|---|---|---|
| CLI 入口 | [`src/client/cli.ts#L185-L193`](../src/client/cli.ts#L185-L193) | `run` 命令解析参数、创建文件 logger、调用 `runOne()`，最后把 `RunOutcome` 投影成 `RunReport` |
...
| 报告投影 | [`src/client/reports.ts#L5-L29`](../src/client/reports.ts#L5-L29) | 策略只产出 `RunOutcome`；报告模块用 `RunOptions + RunOutcome` 生成并写出 `RunReport` |
...
- **报告不是判断来源**：策略先根据 SDK 表面和本地状态生成 `RunOutcome`；报告只是 `reports.ts` 对 outcome 的投影。
...
客户端策略：成功路径中，`statusForSuccess()` 根据场景把状态标记为 `completed_slow`。如果 wall timer 先触发，`runWithResilience()` 会用 `AbortSignal.reason` 识别 `wall_timeout`。如果此时没有可见文本，最终报告 `status=aborted_wall_timeout`，并记录 `aborted_wall_timeout` mitigation；如果 SDK/Runner 已经暴露 partial text，则策略优先保护 partial output，返回 `status=partial_returned`、`problem=wall_timeout`，并记录 `tracked_partial_output` 与 `suppressed_retry_after_partial`。换句话说，慢流有持续进度只能避免 idle timeout，不能绕过 wall timeout；而一旦已经有可见输出，自动重试仍会被抑制。
...
| 报告 | 归类为 `idle_timeout` 或 `wall_timeout` | 取决于先触发的是 idle timer 还是 wall timer |
...
| 报告 | 归类为空内容 idle 或 wall/idle abort | 取决于 SDK 表面和先触发的 abort timer |
...
客户端策略：首 token 前有限重试仍失败后，报告 `circuit_opened=true`，记录 `opened_circuit_breaker`，并按 `protocol/baseUrl/model` 写入进程内 circuit breaker 表。相同 provider key 的后续请求会在 preflight 阶段被拦截，报告 `blocked_circuit_breaker` 和 `status=circuit_opened`，不会再调用 SDK Runner。
...
每次运行都会在策略层产生结构化日志事件。当前 CLI 默认注入文件 logger，文件 logger 在收到 `run_finished` 的 `RunOutcome` 事件后，再调用 `buildRunReport(options, outcome)` 生成 `RunReport` 并写出 `reports/<request_id>.json`。这样请求流程只负责调用 SDK 和产出运行结果，不把报告对象作为日志参数，也不把 `RunReport` 作为流程返回类型。
...
| `run_finished` | 最终 `RunOutcome` 已生成，文件 logger 在这里投影成 `RunReport` 并写 JSON |
